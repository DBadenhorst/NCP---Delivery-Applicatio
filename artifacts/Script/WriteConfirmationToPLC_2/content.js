...
try {
    ...
    await log("Retrieved scale data"); 

    Response.ok(plcRequestData);
} catch(err) {
    await log("Error", {
        "error_keyword": "Retrieving scale data",
        "error_message": err
    }); 
...
// Final Weight or Net + Tare = Gross
let grossWeight = parseFloat(plcRequestData.PLC01_Weight_Actual);

// Initial Weight or Vessel Weight
let tareWeight = parseFloat(plcRequestData.PLC_Initial_Weight);

// Gross - Tare = Net or Product Weight
let netWeight = grossWeight - tareWeight;

//+---------------------------------------------+
//| Real Work Descriptions                      |
//+---------------------------------------------+
//| > Net Weight    - Product Weight            |
//| > Tare Weight   - Vessel Weight             |
//| > Gross Weight  - Net Weight + Tare Weight  |
//+---------------------------------------------+
//| NCP Chlorine Filling Descriptions           |
//+---------------------------------------------+
//| > Net Weight    => Actual Tare Weight       |
//| > Tare Weight   => Net Weight               |
//+---------------------------------------------+

let ncpTareWeight = netWeight;
let ncpNetWeight = tareWeight;
let ncpGrossWeight = grossWeight;

try {
    sapUpdateData = await sapUpdatePromise({
        equipment: plcRequestData.PLC01_Equnr_from_PLC.split(",")[0],
        tare_weight: ncpTareWeight,
        nett_weight: ncpNetWeight,
        gros_weight: ncpGrossWeight
    }, headers["neptune-user"]);
      
    await log("Filling weights confirmed", {
        "tare_weight": ncpTareWeight,
        "nett_weight": ncpNetWeight,
        "gros_weight": ncpGrossWeight
    }); 
    ...
    await log("Error", {
        ...
        "nett_weight": ncpNetWeight,
        "gros_weight": ncpGrossWeight,
        "error_keyword": "Confirming filling weights",
        "error_message": err
    }); 
